        ...
        switch (args[0]) {
            ...
            case "history":
                        ...
                        getDate(item.token).then(date => {
                            ...
                            log(
                                primary(
                                    `[+] ${item.token} - (${
                                        item.ign
                                    }) ${`expires in ${expiry[0]} days`} [generated ${pms(
                                        Date.now() - item.timeGenerated,
                                        { compact: true }
                                    )} ago]`
                                )
                            );
                ...
                if (result.current && result.last) {
                    log(
                        primary(
                            `viewing page ${result.current}/${result.last} (${arr.length} accounts were found)`
                        )
                    );
                } else {
                    log(
                        primary(
                            `viewing page 1/1 (${arr.length} accounts were found)`
                        )
                    );
                }
                if (!conf.get(`history-type`)) {
                    log(
                        primary(
                            "(you can show more in the history by using 'config')"
                        )
                    );
                }
                break;
